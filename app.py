...
if st.session_state.messages and "This concludes our interview" in st.session_state.messages[-1]["content"]:
    ...
    if st.button("Submit Full Report"):
        try:
            with st.spinner("Submitting test data to Jotform..."):
                # --- We will create a simple, hard-coded test submission ---
                test_data = {
                    JOTFORM_FIELD_MAPPING["respondentName"]: "Test Submission Name"
                }
                
                submission_payload = {f'submission[{key}]': str(value) for key, value in test_data.items()}
                url = f"https://api.jotform.com/form/{JOTFORM_FORM_ID}/submissions?apiKey={JOTFORM_API_KEY}"
                
                st.info("DEBUGGING: Sending simplified data to Jotform")
                st.json(submission_payload)

                response = requests.post(url, data=submission_payload)

                st.info("DEBUGGING: Response from Jotform")
                st.json(response.json())

                if response.status_code in [200, 201]:
                    st.success("Test submission sent! Please check your Jotform.")
                else:
                    st.error("Test submission failed.")
